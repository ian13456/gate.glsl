class Perceptron {
  ...
  step = () => {
    ...
    for (let j = 0; j < this.samples.length; j++) {
      ...
      const dEdW1 =
        this.derivativeCrossEntropy(yHat, target) * this.derivativeSigmoid(yHat) * sample[0]
      const dEdW2 =
        this.derivativeCrossEntropy(yHat, target) * this.derivativeSigmoid(yHat) * sample[1]
    ...
    if (avgError < this.maxError) {
      ...
      console.log(`shit: ${this.test2([0.9, 0.9])}`)
    }
  ...
  sigmoid2 = z => {
    ...
    else return 0.5 + z * (1.0 - Math.abs(z) * 0.5)
  }
}
